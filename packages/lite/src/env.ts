 ...
 */
export function create<T extends z.ZodObject>(
  schema: T,
  data: unknown,
  ...
  if (options.fatal) {
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        ...
        throw new Error(
          "Invalid environment variables. The environment variable did not pass the schema validation. Check if you have set all required environment variables correctly."
        );
      }
      throw error;
    }
  ...
  if (!result.success) {
    ...
    for (const key of keys.options) {
      if (!(key in partialEnv.data)) {
        // This is for those entries in schema that are "Required" but missing in the .env
...
export function prettifyEnv(env: Record<string, any>): string | undefined {
  if (!env || Object.keys(env).length === 0) {
    ...
    logger.warn(
      "No environment variables to print. Make sure the env is complete and valid."
    );
    return;
  }
